...
Before installing R packages, you have to install devtools package and msgpack-r packagewith `install.packages("devtools"); devtools::install_github("crowding/msgpack-r")`. 
Or, you can use [embulk](https://www.embulk.org/) and [embulk-output-td](https://github.com/treasure-data/embulk-output-td) plugin. To enable embulk, ensure PATH for `embulk` command to be set appropriately. 
